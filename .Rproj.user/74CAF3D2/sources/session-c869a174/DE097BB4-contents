...
# Version sans dépendance à des coordonnées externes
# =============================================================================
...
load_data <- function() {
  ...
  # Rendements Module 3 (on espère que les coordonnées sont déjà dedans)
  rend_path <- file.path(root_dir, "figures", "rendement_par_grappe.csv")
  if (file.exists(rend_path)) {
    data$rendement <- read.csv(rend_path)
    # Si le fichier ne contient pas lat/lon, on tente de joindre depuis prix
  ...
  # Prix Module 4 (avec ou sans coordonnées)
  prix_path <- file.path(root_dir, "figures", "prix_par_grappe.rds")
  if (file.exists(prix_path)) {
    ...
    tabPanel(
      ...
      fluidRow(
        ...
        column(8,
               h4("Indicateurs clés"),
               fluidRow(
                 valueBoxOutput("vbox_conso"),
                 valueBoxOutput("vbox_surface"),
                 valueBoxOutput("vbox_revenu")
               )
        )
      ),
      fluidRow(
...
server <- function(input, output, session) {
  ...
  output$vbox_conso <- renderValueBox({
    valueBox("79,2 %", "Ménages consommant le Mil", icon("utensils"), color = "blue")
  })
  output$vbox_surface <- renderValueBox({
    valueBox("51,8 %", "Superficie agricole occupée", icon("tractor"), color = "green")
  })
  output$vbox_revenu <- renderValueBox({
    valueBox("14,5 %", "Part du revenu agricole total", icon("money-bill-wave"), color = "yellow")
  })
  ...
  output$map_sa <- renderLeaflet({
    ...
    # Utiliser les régions avec des coordonnées approximatives
