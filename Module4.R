...
# =============================================================================
# Génération de la carte des scores FIES par région
# =============================================================================

# Charger la base du Module 2
base <- readRDS("figures/base_analyse_module2.rds")

# Convertir la colonne region en caractère (si elle est en facteur ou labelled)
base <- base %>%
  mutate(region = as.character(haven::as_factor(region)))  # gestion des labels Stata

# Agrégation par région (moyenne pondérée du score FIES)
fies_region <- base %>%
  group_by(region) %>%
  summarise(
    fies_moy = weighted.mean(fies_score, hhweight, na.rm = TRUE),
    .groups = "drop"
  )

# Coordonnées approximatives des régions du Niger (centroïdes)
coords_region <- data.frame(
...
# Joindre les coordonnées
fies_carte <- fies_region %>%
  left_join(coords_region, by = "region")
...
if (requireNamespace("maps", quietly = TRUE)) {
  library(ggplot2)
  p <- ggplot() +
    borders("world", regions = "Niger", fill = "#E6E6E6", color = "darkgray") +
    geom_point(data = fies_carte,
               aes(x = lon, y = lat, color = fies_moy, size = fies_moy)) +
    scale_color_gradient(low = "green", high = "red", name = "FIES moyen") +
    scale_size_continuous(range = c(3, 10), guide = "none") +
    labs(title = "Score FIES moyen par région",
         x = "Longitude", y = "Latitude") +
    theme_minimal() +
    theme(panel.grid = element_line(color = "grey90"))
  ggsave("figures/carte_fies_region.png", p, width = 8, height = 6, dpi = 300)
  print(p)
} else {
  # Fallback : graphique à barres si maps n'est pas disponible
  library(ggplot2)
  p <- ggplot(fies_region, aes(x = region, y = fies_moy, fill = fies_moy)) +
    geom_col() +
    scale_fill_gradient(low = "green", high = "red", name = "FIES moyen") +
    labs(title = "Score FIES moyen par région",
         x = "Région", y = "Score FIES moyen") +
    theme_minimal() +
...
cat("✔ Carte 'carte_fies_region.png' générée dans le dossier figures/\n")
